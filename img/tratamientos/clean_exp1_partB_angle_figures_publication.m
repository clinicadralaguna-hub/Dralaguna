...
%   1) Remove all large annotation boxes ("final: ...", "first done ...").
%      Final numerical values are already reported in the Part B table.
...
for k = 1:numel(files)
    ...
    %% ---------------------------------------------------------------
    % Remove large annotation text boxes.
    % These are redundant because the numerical results are in the table.
    %% ---------------------------------------------------------------
    ...
    % Some .fig versions may store annotations as ordinary text objects.
    txt = findall(fig, 'Type', 'text');
    for j = 1:numel(txt)
        try
            raw = get(txt(j), 'String');
